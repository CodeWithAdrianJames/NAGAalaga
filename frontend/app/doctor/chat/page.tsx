...
export default function DoctorChatPage() {
  ...
  const parentName = "Maria Santos";
  const parentChild = "Child: Juan Santos, 7 yrs";
  ...
  const sendChat = async () => {
    ...
    /* optimistic update */
    const optimistic = { ...chatData, id: `temp-${Date.now()}-${Math.random()}` } as any;
  ...
  const MessageStatus = ({ status }: { status?: string }) => {
    if (status === "read")
      return <FaCheckDouble className="text-[#E6E1F2]" size={12} />;
    if (status === "delivered")
      return <FaCheckDouble className="text-[#E6E1F2]/60" size={12} />;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#F2EFF9] via-[#E6E1F2] to-[#F2EFF9] flex items-center justify-center p-2 md:p-4">
      <div className="w-full max-w-5xl h-[95vh] bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col relative border border-[#3F2870]/10">
        ...
        <div className="bg-gradient-to-r from-[#3F2870] via-[#4a3380] to-[#5a3d99] px-4 md:px-6 py-4 flex items-center justify-between shadow-xl relative overflow-hidden">
          ...
          <div className="flex items-center gap-3 md:gap-4 relative z-10">
            <div className="relative">
              <div className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-gradient-to-br from-[#E6E1F2] to-white flex items-center justify-center border-3 border-white shadow-lg transform transition-transform hover:scale-105">
                <FaUserCircle className="text-[#3F2870]" size={26} />
              </div>
              {isOnline && (
                <>
                  <div className="absolute bottom-0 right-0 w-4 h-4 bg-green-400 rounded-full border-2 border-white shadow-md" />
                  <div className="absolute bottom-0 right-0 w-4 h-4 bg-green-400 rounded-full animate-ping" />
                </>
              )}
            </div>

            <div>
              <h2 className="text-white font-bold text-base md:text-lg tracking-wide">{parentName}</h2>
          ...
          <div className="flex items-center gap-2 relative z-10">
            <button onClick={() => setShowSearch(!showSearch)} className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-md flex items-center justify-center transition-all hover:scale-110 active:scale-95">
        ...
        {showSearch && (
          <div className="px-4 py-3 bg-[#F2EFF9] border-b border-[#E6E1F2] animate-slideDown">
            <div className="relative">
              ...
              />
              <button onClick={() => setShowSearch(false)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#3F2870]/50 hover:text-[#3F2870]">
                <FaTimes size={14} />
              </button>
            </div>
          </div>
        ...
        <div className="px-6 py-3 bg-gradient-to-r from-[#E6E1F2] to-[#F2EFF9] border-b border-[#3F2870]/10">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 bg-[#3F2870] rounded-full" />
              <span className="text-xs font-semibold text-[#3F2870] uppercase tracking-wider">{parentChild}</span>
            </div>
            <div className="flex items-center gap-2">
              <FaClipboardList className="text-[#3F2870]/60" size={12} />
              <span className="text-xs font-bold text-[#3F2870]">Case #2847</span>
              <span className="text-xs text-[#3F2870]/60">• Follow-up</span>
            </div>
          </div>
        </div>
                ...
                {messages.map((msg, i) => {
                  ...
                  return (
                    <div key={i} className="animate-fadeIn">
                      <div className={`flex gap-2 ${isDoctor ? "justify-end" : "justify-start"} ${isConsecutive ? "mt-1" : "mt-4"}`}>
                          ...
                          <div className="flex-shrink-0">
                            {showAvatar ? (
                              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#E6E1F2] to-white flex items-center justify-center shadow-md border-2 border-white">
                                <FaUser className="text-[#3F2870]" size={14} />
                              </div>
                            ) : (
                              <div className="w-8" />
                            )}
                        ...
                        {/* bubble */}
                        <div className={`max-w-[75%] md:max-w-[65%] group ${isDoctor ? "animate-slideInRight" : "animate-slideInLeft"}`}>
                          {/* sender label */}
                          {showAvatar && !isDoctor && (
                            <div className="flex items-center gap-2 mb-1.5 ml-1">
                              <span className="text-[10px] font-bold text-[#3F2870] uppercase tracking-wider">{parentName}</span>
                              <span className="text-[9px] text-[#3F2870]/40 font-medium">Parent</span>
                            </div>
                          )}

                          <div className={`relative px-4 py-3 shadow-lg hover:shadow-xl transition-all duration-300 ${
                            isDoctor
                            ...
                            {/* footer */}
                            <div className={`flex items-center justify-end gap-1.5 mt-2 ${isDoctor ? "text-white/80" : "text-[#3F2870]/60"}`}>
                              <span className="text-[10px] font-semibold">{formatTime(msg.createdAt)}</span>
                              {isDoctor && <MessageStatus status={msg.status} />}
                            </div>

                            {/* hover reaction */}
                            <div className={`absolute top-0 ${isDoctor ? "left-0 -translate-x-full" : "right-0 translate-x-full"} opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 px-2`}>
            ...
            <div className="flex items-start gap-2 animate-fadeIn">
              ...
              <div className="bg-white px-5 py-4 rounded-2xl rounded-bl-md shadow-lg border-2 border-[#E6E1F2]">
                <div className="flex gap-1.5">
                  <div className="w-2.5 h-2.5 bg-[#3F2870] rounded-full animate-bounce" style={{ animationDelay: "0ms", animationDuration: "1s" }} />
                  <div className="w-2.5 h-2.5 bg-[#3F2870] rounded-full animate-bounce" style={{ animationDelay: "200ms", animationDuration: "1s" }} />
                  <div className="w-2.5 h-2.5 bg-[#3F2870] rounded-full animate-bounce" style={{ animationDelay: "400ms", animationDuration: "1s" }} />
                </div>
              </div>
            </div>
          )}
        ...
        <div className="bg-white border-t-2 border-[#E6E1F2] p-3 md:p-4 shadow-2xl relative">
            ...
            <div className="absolute bottom-full left-4 mb-2 bg-white rounded-2xl shadow-2xl border-2 border-[#E6E1F2] p-4 animate-slideUp z-30">
              ...
              <div className="grid grid-cols-7 gap-2">
                {emojis.map((emoji, idx) => (
                  <button key={idx} onClick={() => handleEmojiClick(emoji)} className="text-2xl hover:scale-125 transition-transform active:scale-95 hover:bg-[#F2EFF9] rounded-lg p-2">
                    {emoji}
                  </button>
                ))}
              </div>
          ...
          {/* attachment menu */}
          {showAttachMenu && (
            <div className="absolute bottom-full left-4 mb-2 bg-white rounded-2xl shadow-2xl border-2 border-[#E6E1F2] p-3 animate-slideUp z-30 min-w-[220px]">
              <button onClick={() => handleFileUpload("image")} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-[#F2EFF9] rounded-xl transition-all group">
              ...
              <button onClick={() => handleFileUpload("document")} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-[#F2EFF9] rounded-xl transition-all group">
              ...
              <button onClick={() => handleFileUpload("lab")} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-[#F2EFF9] rounded-xl transition-all group">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-emerald-500 to-teal-500 flex items-center justify-center group-hover:scale-110 transition-transform">
                  <FaClipboardList className="text-white" size={16} />
                </div>
                <span className="text-sm font-semibold text-[#3F2870]">Lab Reports</span>
              </button>
            </div>
          )}
      ...
      <style jsx>{`
        ...
        ::-webkit-scrollbar-track    { background: #F2EFF9; border-radius: 10px; }
        ::-webkit-scrollbar-thumb    { background: linear-gradient(180deg,#3F2870,#5a3d99); border-radius: 10px; }
        ::-webkit-scrollbar-thumb:hover { background: linear-gradient(180deg,#5a3d99,#3F2870); }
